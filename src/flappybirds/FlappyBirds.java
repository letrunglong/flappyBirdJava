...
public class FlappyBirds extends GameScreen{
    ...
    private int CurrentSreen = BEGIN_SCREEN;
    ...
    public void GAME_UPDATE(long deltaTime) {
        ...
        if(CurrentSreen == BEGIN_SCREEN){
            resetGame();
        }else if(CurrentSreen == GAMEPLAY_SCREEN){
           ...
           for(int i =0; i<ChimneyGroup.SIZE;i++){
               if(bird.getRect().intersects(chimneyGroup.getChimney(i).getRect())){
                   if(bird.getLive()==true) bird.bupSound.play();
                   bird.setLive(false);
           ...
           for(int i = 0;i< chimneyGroup.SIZE;i++){
            ...
            if(bird.getPosY() + bird.getH() > ground.GetYGround()) CurrentSreen = GAMEOVER_SCREEN;
    ...
    public void GAME_PAINT(Graphics2D g2) {
        ...
        if(CurrentSreen == GAMEOVER_SCREEN){
            g2.setColor(Color.red);
    ...
    public void KEY_ACTION(KeyEvent e, int Event) {
            if(Event == KEY_PRESSED){
                ...
                if(CurrentSreen == BEGIN_SCREEN){
                    ...
                    CurrentSreen = GAMEPLAY_SCREEN;
                    
                }else if(CurrentSreen == GAMEPLAY_SCREEN){
                    if(bird.getLive())  bird.fly();
                    
                }else if(CurrentSreen == GAMEOVER_SCREEN){
                    CurrentSreen = BEGIN_SCREEN;
                }
